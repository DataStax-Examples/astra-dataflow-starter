 ...
 *     -Pdirect-runner
 */
public class LoadStaticDataIntoAstra {

  /**
   * Logger for the class.
   */
  private static final Logger LOGGER = LoggerFactory.getLogger(LoadStaticDataIntoAstra.class);
  ...
  private static AstraIO.Write<SimpleDataEntity> setupAstraWrite(AstraPipelineOptions astraOptions) {
    ...
    try {
      if (scbParam.startsWith("gs")) {
        ...
        writeToAstra= writeToAstra.withSecureConnectBundleURL(new URL(scbParam));
      } else {
        ...
        writeToAstra= writeToAstra.withSecureConnectBundleFile(new File(scbParam));
      }
    } catch(Exception e) {
